...
function CollegeSchedule({ enrolledCourses }) {
    ...
    const addTimeSlot = (courseIndex) => {
        ...
        updatedCourses[courseIndex].timeSlots.push({ day: "", timeslot: "", type: "Lecture" });
        setCourses(updatedCourses);
    };
    ...
    const handleUpdateSchedule = async (e) => {
        ...
        if (validateForm()) {
            try {
                ...
                const response = await axios.post("http://localhost:5100/schedule/update-collegeSchedule", {
                    userId,
                ...
                setErrors({ 
                    submit: error.response?.data?.error || "Failed to update schedule. Please try again." 
                });
            } finally {
                setUpdating(false);
            }
